...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("Search Yourself ‼️", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.send_cached_media(#chat_id=query.from_user.id,
                    file_id=file_id,
                    caption=f_caption,
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            InlineKeyboardButton('<-', callback_data='help'),
            InlineKeyboardButton('Only Admins!', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('<-', callback_data='help'),
            InlineKeyboardButton('♻️ Refresh again?', callback_data='rfrsh')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        total = await Media.count_documents()
        users = await db.total_users_count()
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"🎵 {file.file_name}", callback_data=f'{pre}#{file.file_id}' #[{get_size(file.file_size)}]
                ),
            ]
            for file in files
        ]
...
async def advantage_spell_chok(client, msg):
    ...
    btn.append([InlineKeyboardButton(text="×", callback_data=f'spol#{reqstr1}#close_spellcheck')])
    spell_check_del = await msg.reply_text(
        text="<b>Cʜᴏᴏꜱᴇ ᴛʜᴇ ᴄᴏʀʀᴇᴄᴛ ᴏɴᴇ ʙᴇʟᴏᴡ.</b>",
        reply_markup=InlineKeyboardMarkup(btn),
        reply_to_message_id=msg.id
    )
